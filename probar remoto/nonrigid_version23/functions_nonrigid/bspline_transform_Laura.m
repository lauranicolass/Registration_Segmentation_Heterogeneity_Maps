function [I,T]=bspline_transform_Laura(O,I,Spacing,mode)
...
%            4: nearest neightbour interpolation and outside pixels set to nearest pixel
%
% outputs,
...
if(size(I,3)<4)
    ...
    if(nargout > 1 )
        [I,Tx,Ty]=bspline_transform_2d_double_Laura(double(O(:,:,1)),double(O(:,:,2)),I,double(Spacing(1)),double(Spacing(2)),double(mode));
        T(:,:,1)=Tx; 
        T(:,:,2)=Ty;
    else
        I=bspline_transform_2d_double_Laura(double(O(:,:,1)),double(O(:,:,2)),I,double(Spacing(1)),double(Spacing(2)),double(mode));
        T=0;
    end
else
    if(isa(I,'double'))
